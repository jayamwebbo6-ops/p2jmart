...
const Enquiries = () => {
  ...
  const filteredEnquiries = enquiries.filter(e => {
    const matchesSearch = 
      ...
      (e.subject || '').toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
      (e.message || '').toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
      (e.email || '').toLowerCase().includes(debouncedSearchQuery.toLowerCase());
      
    const matchesStatus = 
      filterStatus === 'ALL' || 
      (filterStatus === 'READ' && e.read) || 
      (filterStatus === 'UNREAD' && !e.read);

    return matchesSearch && matchesStatus;
  });

  // Adjust active selection if current active enquiry is filtered out
  useEffect(() => {
    if (filteredEnquiries.length > 0) {
      const activeIsStillPresent = filteredEnquiries.some(e => e.id === activeEnquiryId);
      if (!activeIsStillPresent) {
        setActiveEnquiryId(filteredEnquiries[0].id);
      }
    } else {
      setActiveEnquiryId('');
    }
          ...
          <div className="p-4 border-b border-gray-100 flex items-center gap-2 bg-gray-55/30">
            ...
            <div className="relative w-28 flex-shrink-0">
              <select
                ...
                className="w-full bg-gray-50 border border-gray-200 text-gray-700 text-xs rounded-lg pl-3 pr-8 py-2 appearance-none focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all cursor-pointer font-semibold uppercase tracking-wider"
              >
                <option value="ALL">All</option>
                <option value="UNREAD">Unread</option>
                <option value="READ">Read</option>
              </select>
              <div className="absolute right-2.5 top-2.5 text-gray-450 pointer-events-none flex items-center">
