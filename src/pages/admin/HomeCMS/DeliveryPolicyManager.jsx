...
const DeliveryPolicyManager = ({ sections = [], setSections, onSave, isSaving, askConfirmation }) => {
  ...
  const handleRemoveSection = (sectionId) => {
    askConfirmation({
      ...
      message: "Are you sure you want to delete this delivery policy section? This action cannot be undone.",
      onConfirm: () => {
  ...
  const handleSaveAllPolicies = () => {
    onSave({ deliveryPolicy: sections });
  };
        ...
        {sections.length > 0 && (
          <div className="flex justify-end pt-2">
            <button
              ...
              disabled={isSaving}
              className="bg-[#00213d] hover:bg-black text-white font-bold text-xs uppercase tracking-wider px-6 py-3 rounded-xl shadow-xs transition-all active:scale-95 cursor-pointer disabled:opacity-50"
            >
              {isSaving ? 'Saving...' : 'Save Delivery Policy'}
            </button>
          </div>
        )}
